...
class MyLocationListener implements BDLocationListener {
	...
	public void onReceiveLocation(BDLocation location) {
		...
		if (location == null
				|| (location.getLocType() != 61 && location.getLocType() != 65 && location
						.getLocType() != 161)) {
			if (BaiduLocation.handler != null) {
				BaiduLocation.handler.sendObject(BaiduLocation
						.getBufLocation(context));
				if (!BaiduLocation.isAuto) {
			...
			if (location.getCity() == null) {
				...
				model.City = location.getCity().replace("省", "")
						.replace("市", "").replace("特别行政区", "");
			}
			if (location.getProvince() == null) {
				model.Province = "";
			} else {
				model.Province = location.getProvince().replace("省", "")
						.replace("市", "").replace("特别行政区", "");
			}
...
public class BaiduLocation {
	...
	static LocationClient createBaiDuLocation(Context context) {
		...
		locationClient
				.registerLocationListener(new MyLocationListener(context));
		...
		option.setProdName("com.mira");
		locationClient.setLocOption(option);
		return locationClient;
	}

	public static void getLocation(Context context,
			HandlerEvent<BaiDuLocationModel> handler) {
		getLocation(context, handler, false);
	}

	public static void getLocation(Context context,
			HandlerEvent<BaiDuLocationModel> handler, boolean isAuto) {
	...
	public static BaiDuLocationModel getBufLocation(Context context) {
		try {
			...
			BaiDuLocationModel model = new BaiDuLocationModel();
			SharedPreferences sp = context.getSharedPreferences("Location",
	...
	public static void setBufLocation(Context context, BaiDuLocationModel model) {
		try {
			Editor ed = context.getSharedPreferences("Location",
	...
	public static TestModel getWeather(final Context context) {
		if (!isWeather) {
			...
			new Thread(new Runnable() {
				...
				@Override
				public void run() {
					BaiDuLocationModel model = BaiduLocation
							.getBufLocation(context);
					BaiduLocation.getLocation(context,
							new HandlerEvent<BaiDuLocationModel>() {
								@Override
								public void handleMessage(
										BaiDuLocationModel result) {
					...
					try {
						String text = MRCommon
								.getHttpData("http://api.map.baidu.com/telematics/v3/weather?location="
										+ model.Lng
										+ ","
										+ model.Lat
										+ "&output=json&;ak=C849c6992c5232a86e0f6e4426e7ce29&coord_type=wgs84&qq-pf-to=pcqq.discussion");
						Gson gson = new Gson();
						MRWeatherModel weatherModel = gson.fromJson(text,
								MRWeatherModel.class);
						if (weatherModel != null
								&& weatherModel.results != null) {
