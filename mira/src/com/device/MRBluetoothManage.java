...
public class MRBluetoothManage {
	...
	public static synchronized int Init(Context context,MRBluetoothEvent event)
	{
		...
		final BluetoothManager bluetoothManager =
		        (BluetoothManager) MRBluetoothManage.context.getSystemService(Context.BLUETOOTH_SERVICE);
	...
	public static synchronized void scanAndConnect()
	{
		...
		bluetoothAdapter.startLeScan(new LeScanCallback() {
			...
			public void onLeScan(BluetoothDevice arg0, int arg1, byte[] arg2) {
				...
				if(!isScanOK && arg0.getName()!=null && arg0.getName().equals(DeviceName))
				{
					isScanOK=true;
					stopScan();
					connect(arg0);
		...
		scanTimer.schedule(new TimerTask() {
			...
			public void run() {
				synchronized (deviceHashMap) {
					...
					Iterator<Entry<String, BluetoothDevice>> iter=deviceHashMap.entrySet().iterator();
					while(iter.hasNext())
					{
